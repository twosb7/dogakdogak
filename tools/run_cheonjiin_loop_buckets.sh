...
LOG_DIR="$ROOT_DIR/build/cheonjiin-loop-logs"
WORK_ROOT="$ROOT_DIR/build/cheonjiin-loop-workdirs"
mkdir -p "$LOG_DIR"
mkdir -p "$WORK_ROOT"
...
for bucket in "${buckets[@]}"; do
  ...
  (
    cd "$repo_dir"
    CHEONJIIN_LOOP=true ./gradlew :app:testDebugUnitTest \
      --tests "helium314.keyboard.latin.InputLogicTest.cheonjiinEditingLoopBucket${bucket}"
  ) >"$log_file" 2>&1 &
  pids+=($!)
done
